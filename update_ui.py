...
def update_css():
    ...
    transition: background-color 0.3s ease, border-color 0.3s ease;
        ...
        for old, new in dark_theme_replacements.items():
        ...
        for old, new in light_theme_replacements.items():
            css = css.replace(old, new)
...
@keyframes pop-score {
    0% { transform: scale(1); }
    50% { transform: scale(1.15); font-weight: 800; text-shadow: 0 0 15px currentColor; }
...
"""
        ...
        if "/* --- Animações Premium --- */" not in css:
            css += animation_css

        with open('styles.css', 'w', encoding='utf-8') as f:
            f.write(css)
        print("Updated styles.css with Premium UI variables and animations!")
